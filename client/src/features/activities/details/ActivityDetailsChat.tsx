...
const ActivityDetailsChat = observer(function ActivityDetailsChat() {
  ...
  const addComment = async (data: FieldValues) => {
    try {
      // The ['SendComment'] has to be [exactly] like that. VVV
      // Because that's what we [called] it in the [CommentHub.cs] [Class]. 
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLDivElement>) => {
    // So we're [only] going to [submit] the [comments] 
    // if the ['Enter'] [key] is [pressed], [without] also the ['shift'] [key]
